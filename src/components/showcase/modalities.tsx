    ...
    return (
        <section className='w-full h-fit flex flex-col gap-4 bg-obsidian items-center justify-center'>
            <header className='w-full gap-4 h-fit items-center px-7.5 justify-center flex flex-col'>
                <h1 className='font-bebas-neue text-3xl text-forge-white text-center'>Aqui, cada treino tem um <span className='text-iron-gold'> propósito</span>.</h1>
                <h2 className='font-inter text-md text-silver text-center'>Não importa seu nível. Existe um treino certo pra você começar hoje.</h2>
            </header>
            <article className='w-full px-7.5 h-fit flex flex-col items-center justify-center gap-4'>
                <div className='items-center w-full h-fit justify-center grid grid-cols-1 gap-4'>
                    {modalities.map((m, i) => (
                        <ModalitieCard title={m.title} key={i} cta={m.cta} benefits={m.benefits} description={m.description} link={m.link} poster={m.poster} />
                    ))}
                </div>
                <article className='w-full gap-4 h-fit flex flex-col items-center justify-center'>
                    <h1 className='font-inter text-xl text-forge-white uppercase font-bold text-center'>Independente da escolha, o importante é <span className='text-iron-gold'> começar</span>.</h1>
                    <h2 className='font-inter text-base text-silver text-center'>Dê o primeiro passo hoje e sinta a diferença já nos primeiros treinos.</h2>
                    ...
                    <p className='font-inter text-silver underline text-center'> Sem experiência? Sem problema. Você começa do seu nível.</p>
                </article>
            </article>
        </section>
    )
